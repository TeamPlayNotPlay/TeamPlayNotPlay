        ...
        if message.author.guild_permissions.administrator:
            ...
            try:
                ...
                if result is None:
                    ...
                    embed = discord.Embed(title=' 등록 실패(관리자에게 문의 해주세요!)', description='이미 등록된 유저입니다!', color=0xFF0000)
                    await message.channel.send(embed=embed)
                    return
                ...
                embed = discord.Embed(title=' 오류발생', description=f'오류가 발생하였습니다!\n`{str(e)}`', color=0xFF0000)
                await message.channel.send(embed=embed)
        else:
            await message.channel.send(f'{message.author.mention} 권한이 부족하니, 계속 시도하지마십시오.')
